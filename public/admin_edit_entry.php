 ...
 * Diese Seite erlaubt Administratoren, jeden Eintrag zu bearbeiten
 */
...
$stmt = $pdo->prepare("
    SELECT e.id, e.titel, e.beschreibung, e.benutzer_id,
...
// Wenn kein Eintrag gefunden wurde, zum Admin-Dashboard umleiten
if (!$eintrag) {
    umleiten_zu('admin_dashboard.php?error=entry_not_found');
}
...
if (isset($_GET['delete_image']) && is_numeric($_GET['delete_image'])) {
    ...
    // Prüfen, ob das Bild zum Eintrag gehört
    $stmt = $pdo->prepare("
        SELECT dateiname, ist_thumbnail 
        FROM eintrag_bilder 
        WHERE id = :bild_id AND eintrag_id = :eintrag_id
    ");
    $stmt->execute([
        'bild_id' => $bild_id,
    ...
    if ($zu_loeschendes_bild) {
        ...
        umleiten_zu('admin_edit_entry.php?id=' . $eintrag_id . '&image_deleted=1');
    }
}
...
if (isset($_GET['set_thumbnail']) && is_numeric($_GET['set_thumbnail'])) {
    ...
    if ($stmt->fetchColumn()) {
        ...
        try {
            ...
            umleiten_zu('admin_edit_entry.php?id=' . $eintrag_id . '&thumbnail_set=1');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($fehler)) {
        ...
        try {
            ...
            umleiten_zu('admin_dashboard.php?success=entry_updated');
        } catch (PDOException $e) {
        ...
        <div class="col-md-8 mx-auto">
            <div class="card">
                <div class="card-header bg-primary text-white">
                    <h4>Eintrag bearbeiten (Admin-Modus)</h4>
                    <small class="text-white">Erstellt von: <?php echo htmlspecialchars($eintrag['vorname'] . ' ' . $eintrag['nachname']); ?></small>
                </div>
                <div class="card-body">
                    ...
                    <?php if ($success): ?>
                        ...
                        <script>
                            setTimeout(function() {
                                window.location.href = 'admin_dashboard.php?success=entry_updated';
                            }, 2000);
                        </script>
                    <?php else: ?>
                                        ...
                                        <div class="card">
                                            ...
                                            <div class="card-body p-2">
                                                <div class="d-flex justify-content-between align-items-center">
                                                    ...
                                                    <?php else: ?>
                                                        <a href="admin_edit_entry.php?id=<?php echo $eintrag_id; ?>&set_thumbnail=<?php echo $bild['id']; ?>" class="btn btn-outline-success btn-sm">Als Thumbnail</a>
                                                    <?php endif; ?>
                                                    <a href="admin_edit_entry.php?id=<?php echo $eintrag_id; ?>&delete_image=<?php echo $bild['id']; ?>" class="btn btn-outline-danger btn-sm" onclick="return confirm('Bild wirklich löschen?');">Löschen</a>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
